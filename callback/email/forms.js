...
var ko_document_ready = function(force){
    ...
    Array.from(document.forms).forEach(function(form){
        form.addEventListener('submit', function(e){
            ...
            ko_ajax.post('./callback/email/callback.php', data, function(r){
...
var ko_classes_ajax = function(){
	...
	this.script_add = function(url){
		...
		if(container == null){
			(function(t, url){
				setTimeout(function(url){ t.script_add(url); }, 50);
			})(this, url);
			return true;
		}
	...
	this.send = function(url, data, callback, async, cache, content_type, options){
	...
	this.jsonp = function(url, data, callback, async, cache, options){
		...
		r = this.send(url + (query.length ? '?' + query : ''), null, callback, async, cache, 'jsonp', options);
		return r;
	};
	this.get = function(url, data, callback, async, cache, content_type, options){
		var r = false;
		var query = data;
		if(!(data instanceof FormData)){
			query = ko_f.http_build_query(data || {});
		}
		r = this.send(url + (query.length ? '?' + query : ''), null, callback, async, cache, content_type, options);
		return r;
	};
	this.post = function(url, data, callback, async, cache, content_type, options){
		var r = false;
		var query = data;
